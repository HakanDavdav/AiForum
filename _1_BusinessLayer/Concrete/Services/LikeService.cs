...
namespace _1_BusinessLayer.Concrete.Services
{
    public class LikeService : AbstractLikeService
    {
        public LikeService(
            ...
            : base(notificationQueryHandler, entryQueryHandler, postQueryHandler, userQueryHandler, botQueryHandler, followQueryHandler, likeQueryHandler, genericCommandHandler, mailEventFactory, notificationEventFactory, queueSender, unitOfWork)
        {
        }
        ...
        public override async Task<IdentityResult> LikeEntry(int entryId, int userId)
        {
            ...
            var entry = await _entryQueryHandler.GetBySpecificPropertySingularAsync(q => q.Where(e => e.EntryId == entryId).Include(e => e.OwnerUser).Include(e => e.OwnerBot).Include(e => e.Likes));
            if (entry == null)
        ...
        public override async Task<IdentityResult> LikePost(int postId, int userId)
        {
            ...
            var post = await _postQueryHandler.GetBySpecificPropertySingularAsync(q => q.Where(p => p.PostId == postId).Include(p => p.OwnerUser).Include(p => p.OwnerBot).Include(p => p.Likes));
            if (post == null)
        ...
        public override async Task<IdentityResult> UnlikeEntry(int userId, int likeId)
        {
            ...
            try
            {
                var like = await _likeQueryHandler.GetBySpecificPropertySingularAsync(q => q.Where(l => l.LikeId == likeId && l.OwnerUserId == userId).Include(l => l.Entry).Include(l => l.OwnerUser));
                if (like == null)
                ...
                await _genericCommandHandler.DeleteAsync<Like>(like);
                await _genericCommandHandler.SaveChangesAsync();
        ...
        public override async Task<IdentityResult> UnlikePost(int userId, int likeId)
        {
            ...
            try
            {
                var like = await _likeQueryHandler.GetBySpecificPropertySingularAsync(q => q.Where(l => l.LikeId == likeId && l.OwnerUserId == userId).Include(l => l.Post).Include(l => l.OwnerUser));
                if (like == null)
                    return IdentityResult.Failed(new NotFoundError("Like not found or owner is not you"));
                if (like.Post == null)
                    return IdentityResult.Failed(new NotFoundError("Post not found"));
